<?php
	...
	$use_machines = new UseSaleMachine();

	empty($use_machines->getToInvoice($invoice->getInvoiceId())) ? '' : array_push($detalles,$use_machines->getToInvoice($invoice->getInvoiceId()));

	$useSoftwareDesign = new UseSoftwareDesign();

	empty($useSoftwareDesign->getToInvoice($invoice->getInvoiceId())) ? '' : array_push($detalles,($useSoftwareDesign->getToInvoice($invoice->getInvoiceId())));

	if($invoice->getUseType() == 'S'){
		$html = '<!DOCTYPE html> 
		<html lang="es">
		<head>
			<meta charset="utf-8" />
			...
			<div class="invoice-box">
				...
				<table style="border: 1px solid rgb(141, 141, 141);">
					<tbody>
						...
						<tr class="item">
							<td style="text-align: center;">Servicios proporcionados por el Fab Lab UP CIDETE</td>
